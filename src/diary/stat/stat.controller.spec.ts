...
describe('Stat Controller', () => {
  ...
  describe('getRevenue', () => {
    it('should success', async () => {
      const from = new Date('2020-10-25T10:18:10.502Z');
      const to = new Date('2021-10-25T10:18:10.502Z');
      const revenue = { income: 10, outcome: 20 };
      statService.getRevenue.mockResolvedValueOnce(revenue);

      await expect(controller.getRevenue(from, to)).resolves.toEqual(revenue);
      expect(statService.getRevenue).toHaveBeenCalledWith(from, to);
    });
  });
});
